...
@Time : 2020/11/25 下午4:36
@Des  : mysql表结构自动生成golang struct (康存版本）
...
func CreteModel(table string){
	...
	fileName := rr+"Model"
	//modelFilePath := "/Users/uplook/gomodcode/GoInduction/mysqldb/model/model.go" // 暂时写死的 ，后续添加到config中
	//f,err := os.OpenFile(kcgin.WorkPath + "/common/models/" + fileName+".go",os.O_CREATE, 0777)
	workPath := "/Users/uplook/gomodcode/GoInduction"
	f,err := os.OpenFile(workPath + "/mysqldb/model/" + fileName + ".go",os.O_CREATE, 0777)
	if err !=nil{
	...
	strContent += `package model`   // `package models`
	strContent += "\n\n"
	strContent +=
		`import(
	)`;
